...
Deno.test("UserAuth Concept: Principle Trace", async () => {
  ...
  try {
    ...
    const duplicateRegisterResult = await userAuth.register({
      username: "alice",
    ...
    assertExists(
      (loginResult as { token: string }).token,
      "Login should return a session token.",
    );
    const aliceToken = (loginResult as { token: string }).token;
    console.log(
      `   Success: Alice logged in and received token: ${aliceToken}`,
    );
    ...
    const userFromResult = await userAuth._getUserFromToken({
      token: aliceToken,
    });
    ...
    assertObjectMatch(
      userFromResult[0],
      { user: aliceId },
      "Query should return the correct user ID.",
    );
    console.log("   Success: Token resolves to correct user ID.");

    console.log("\n-> Query: _getUsernameFromToken({ token: aliceToken })");
    const usernameFromResult = await userAuth._getUsernameFromToken({
      token: aliceToken,
    });
    assertEquals(usernameFromResult.length, 1);
    assertObjectMatch(
      usernameFromResult[0],
      { username: "alice" },
      "Query should return the correct username.",
    );
    ...
    const postLogoutUserResult = await userAuth._getUserFromToken({
      token: aliceToken,
    });
    assertEquals(postLogoutUserResult.length, 1);
    assertObjectMatch(
      postLogoutUserResult[0],
      { error: "Invalid token." },
      "Query with a logged-out token should fail.",
    );
...
Deno.test("UserAuth Concept: Action Tests", async (t) => {
  await t.step("register action", async (t) => {
    ...
    await t.step("should register a new user successfully", async () => {
      try {
        ...
        console.log(
          "   Effects: A new user is created and its ID is returned.",
        );
        const userInDb = await userAuth.users.findOne({ username: "bob" });
        assertExists(
          userInDb,
          "User 'bob' should exist in the database after registration.",
        );
    ...
    await t.step(
      "should fail to register a user with an existing username",
      async () => {
        try {
          console.log(
          ...
          const usersCount = await userAuth.users.countDocuments({
            username: "charlie",
          });
          assertEquals(
            usersCount,
            1,
            "There should still be only one 'charlie'.",
          );
          console.log(
            "   ✅ Passed: Correctly failed to register a duplicate user.",
          );
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle duplicate username as expected.",
            e,
          );
          throw e;
        }
  ...
  await t.step("login action", async (t) => {
    ...
    await t.step("should log in a user with correct credentials", async () => {
      try {
        ...
        console.log(
          "   Requires: User 'dave' exists with password 'correctpassword'.",
        );
        ...
        const sessionInDb = await userAuth.sessions.findOne({
          _id: (result as { token: string }).token,
        });
        assertExists(
          sessionInDb,
        ...
        console.log(
          "   ✅ Passed: Successfully logged in with correct credentials.",
        );
      } catch (e) {
    ...
    await t.step(
      ...
      async () => {
        try {
          ...
          console.log(
            "   Requires: No user exists with username 'dave' and password 'wrongpassword'.",
          );
          const result = await userAuth.login({
            username: "dave",
          ...
          console.log("   Effects: An error is returned.");
          console.log(
            "   ✅ Passed: Correctly failed to log in with incorrect password.",
          );
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle incorrect password as expected.",
            e,
          );
          throw e;
        }
    ...
    await t.step(
      "should fail to log in with a non-existent username",
      async () => {
        try {
          ...
          console.log("   Effects: An error is returned.");
          console.log(
            "   ✅ Passed: Correctly failed to log in with a non-existent username.",
          );
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle non-existent user as expected.",
            e,
          );
          throw e;
        }
  ...
  await t.step("logout action", async (t) => {
    ...
    await t.step(
      "should handle logout with a non-existent token gracefully",
      async () => {
        try {
          ...
          assertEquals(
            sessionsCount,
            0,
            "No sessions should have been affected.",
          );
          console.log("   ✅ Passed: Handled non-existent token gracefully.");
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle non-existent token as expected.",
            e,
          );
          throw e;
        }
...
Deno.test("UserAuth Concept: Query Tests", async (t) => {
  const [db, client] = await testDb();
  const userAuth = new UserAuthConcept(db);
  const { user: userId } = await userAuth.register({
    username: "grace",
    password: "password123",
  }) as { user: ID };
    ...
    await t.step("_getUserFromToken query", async (t) => {
      ...
      await t.step("should return an error for an invalid token", async () => {
        try {
          ...
          console.log("   Requires: No session exists for 'invalid-token'.");
          const result = await userAuth._getUserFromToken({
            token: "invalid-token",
          });
          assertEquals(result.length, 1);
          ...
          console.log(
            "   ✅ Passed: Correctly returned error for invalid token.",
          );
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle invalid token as expected.",
            e,
          );
          throw e;
        }
    ...
    await t.step("_getUsernameFromToken query", async (t) => {
      await t.step("should return the username for a valid token", async () => {
        try {
          ...
          console.log(
            "   Requires: A session and corresponding user exist for the token.",
          );
          ...
          console.error(
            "   ❌ Failed: Did not return username as expected.",
            e,
          );
          throw e;
        }
      ...
      await t.step("should return an error for an invalid token", async () => {
        try {
          ...
          console.log("   Requires: No session exists for 'invalid-token'.");
          const result = await userAuth._getUsernameFromToken({
            token: "invalid-token",
          });
          assertEquals(result.length, 1);
          assertObjectMatch(result[0], { error: "Invalid token." });
      ...
      await t.step("should handle data inconsistency gracefully", async () => {
        try {
          ...
          await userAuth.sessions.insertOne({
            _id: inconsistentToken,
            user: nonExistentUserId,
          });
          console.log("   Requires: A session exists, but its user does not.");
          const result = await userAuth._getUsernameFromToken({
            token: inconsistentToken,
          });
          assertEquals(result.length, 1);
          assertObjectMatch(result[0], {
            error: "User for session not found.",
          });
          console.log(
            "   Effects: Returns a specific error for data inconsistency.",
          );
          console.log("   ✅ Passed: Handled data inconsistency gracefully.");
        } catch (e) {
          console.error(
            "   ❌ Failed: Did not handle data inconsistency as expected.",
            e,
          );
          throw e;
        }

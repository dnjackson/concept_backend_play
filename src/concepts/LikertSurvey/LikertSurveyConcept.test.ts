...
Deno.test("LikertSurveyConcept: Action `createSurvey`", async (t) => {
  ...
  await t.step(
    "effects: creates a survey and associates it with an owner",
    async () => {
      console.log("  - Testing effect: successfully creates a survey");
      const res = await likert.createSurvey({
        title: "Team Morale",
        owner: userA,
      });
...
Deno.test("LikertSurveyConcept: Action `addQuestion`", async (t) => {
  const [db, client] = await testDb();
  const likert = new LikertSurveyConcept(db);
  const { survey } = await likert.createSurvey({
    title: "Team Morale",
    owner: userA,
  }) as { survey: ID };
  ...
  await t.step("requires: survey must exist", async () => {
    ...
    const res = await likert.addQuestion({
      stem: "A valid question",
      survey: nonExistentSurvey,
    });
  ...
  await t.step(
    "effects: creates a question and associates it with a survey",
    async () => {
      ...
      assert(
        "question" in res,
        "Expected a question ID on successful creation",
      );
      const { question } = res;
      ...
      assert(
        "question" in surveyQuestion,
        "Query for survey questions should not fail",
      );
...
Deno.test("LikertSurveyConcept: Action `respondToQuestion`", async (t) => {
  ...
  const { question } = await likert.addQuestion({
    stem: "Is the UI intuitive?",
    survey,
  }) as { question: ID };

  await t.step("requires: question must exist", async () => {
    console.log("  - Testing requirement: question must exist");
    const res = await likert.respondToQuestion({
  ...
  await t.step(
    "requires: choice must be an integer between 1 and 5",
    async () => {
  ...
  await t.step(
    "effects: creates a response and can overwrite a previous one",
    async () => {
      ...
      assert(
        "counts" in countsRes2,
        "Query for counts should not fail after overwrite",
      );
      assertEquals(
        countsRes2.counts,
        [0, 0, 0, 0, 1],
        "Previous response should be removed",
      );
    },
  );
...
Deno.test("LikertSurveyConcept: Action `removeQuestion`", async (t) => {
  ...
  await t.step(
    "effects: removes the question and its associated responses",
    async () => {
      console.log("  - Verifying state before removal...");
      assertEquals((await likert._getSurveyQuestions({ survey })).length, 1);
      const [countsBefore] = await likert._getQuestionResponseCounts({
        question,
      });
...
Deno.test("LikertSurveyConcept: Principle Test", async () => {
  console.log(
    "Principle: An owner creates a survey, users respond, and the owner analyzes sentiment.",
  );
  const [db, client] = await testDb();
  const likert = new LikertSurveyConcept(db);
  ...
  const q1Res = await likert.addQuestion({
    stem: "Are you happy with the work-life balance?",
    survey,
  });
  ...
  const q2Res = await likert.addQuestion({
    stem: "Is the project challenging enough?",
    survey,
  });
  ...
  console.log(
    "  - Bob responds to Q1 ('work-life balance') with 5 (Strongly Agree).",
  );
  await likert.respondToQuestion({ question: q1, responder: userB, choice: 5 });
  console.log(
    "  - Charlie responds to Q1 ('work-life balance') with 4 (Agree).",
  );
  await likert.respondToQuestion({ question: q1, responder: userC, choice: 4 });
  console.log(
    "  - Bob responds to Q2 ('challenging enough') with 1 (Strongly Disagree).",
  );
  await likert.respondToQuestion({ question: q2, responder: userB, choice: 1 });
  console.log(
    "  - Charlie responds to Q2 ('challenging enough') with 5 (Strongly Agree).",
  );

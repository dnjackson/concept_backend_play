...
Deno.test("LikertSurveyConcept - Principle", async () => {
  console.log(
    "Testing principle: Create survey, get responses, analyze sentiment",
  );
  const [db, client] = await testDb();
  const likertSurvey = new LikertSurveyConcept(db);
  ...
  console.log(
    `    -> Survey '${surveyId}' and Question '${questionId}' created.`,
  );
  ...
  console.log(
    "Principle test successful: The concept correctly aggregates and analyzes responses.",
  );

...
content_id: 444f659c16249f8559bec7da1b2e0949790607039667844c4d18c75a275f0f0a
---
...
Deno.test("LikertSurveyConcept: Action tests", async (t) => {
  ...
  await t.step("respondToQuestion: requires a valid choice (1-5)", async () => {
    ...
    let result = await concept.respondToQuestion({ question: questionId, responder, choice: 0 });
    assertExists(result.error);
    result = await concept.respondToQuestion({ question: questionId, responder, choice: 6 });
    assertExists(result.error);
    result = await concept.respondToQuestion({ question: questionId, responder, choice: 3.5 });
    assertExists(result.error);
  ...
  await t.step("removeQuestion: effects remove the question and its responses", async () => {
    ...
    await concept.removeQuestion({ question: questionId });

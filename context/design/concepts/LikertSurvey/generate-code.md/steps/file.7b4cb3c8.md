...
content_id: 7b4cb3c8e46e96935b07f2b6464b4c4713ed23fccf88ea8da9196f06c3f1bd6c
---
...
export default class LikertSurveyConcept {
  ...
  async _analyzeSentiment({ question }: { question: Question }): Promise<{ sentiment: string } | { error: string }> {
    ...
    const average = sum / n;
    const stdDev = Math.sqrt(scores.map((score) => Math.pow(score - average, 2)).reduce((acc, val) => acc + val, 0) / n);
